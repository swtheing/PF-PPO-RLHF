    ...
    class LLMForSequenceRegression(base_pretrained_model):
        ...
        def forward(
            ...
            if self.training:
                ...
                eos_ind = eos_indices.cpu().tolist()
                input_list = input_ids.cpu().tolist()
                #print(input_list)
                #print(eos_ind)
                for i in range(len(reward)):
                #    print(mode + "\t" + ",".join([str(item) for item in input_list[i][:eos_ind[i][0]]]))
                    print("\n" + mode + "_reward_sw: %f" % reward[i].item())        
